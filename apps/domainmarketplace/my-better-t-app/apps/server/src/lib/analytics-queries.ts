...
async function runQuery<T>(
	...
	if (dataset && typeof (dataset as any).query === "function") {
		...
		if (!response.success) {
			throw new Error(
				`Analytics Engine query failed: ${response.errors?.join(", ") ?? "unknown error"}`,
			);
		}
	...
	if (!bindings.CLOUDFLARE_API_TOKEN || !bindings.CLOUDFLARE_ACCOUNT_ID) {
		throw new Error(
			"Analytics Engine not configured: missing CLOUDFLARE_API_TOKEN or CLOUDFLARE_ACCOUNT_ID",
		);
	}
	...
	const response = await fetch(url, {
		...
		body: JSON.stringify({
			dataset: bindings.CLICKS_DATASET_ID,
			query: sql,
		}),
	});
	...
	if (result.errors?.length) {
		throw new Error(
			`Analytics Engine API error: ${result.errors.map((error) => error.message).join(", ")}`,
		);
	}
...
export async function queryGeoBreakdown(
	...
	const sanitizedHireId = sanitizeValue(hireId);
	const sql = `
		SELECT
			blob5 AS country,
			SUM(_sample_interval) AS clicks
		FROM clicks
		WHERE blob4 = '${sanitizedHireId}'
